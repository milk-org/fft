...
//
errno_t fft_DFT(const char *IDin_name, const char *IDinmask_name, const char *IDout_name, const char *IDoutmask_name,
                double Zfactor, int dir, long kin, imageID *outID)
{
    DEBUG_TRACE_FSTART();
    ...
    printf("DFT (factor %f, slice %ld):  %u input points (%u %u)-> ", Zfactor, kin, NBptsin, NBpixact_iiin,
           NBpixact_jjin);
...
#pragma omp for
        ...
        for (uint32_t pixiiout = 0; pixiiout < NBpixact_iiout; pixiiout++)
        {
            ...
            {
                uint32_t iiin = iiinarrayActive[pixiiin];
                float pha = 2.0 * dir * M_PI * (XinarrayActive[pixiiin] * XoutarrayActive[pixiiout]);
...
#pragma omp for
        ...
        for (uint32_t pixjjout = 0; pixjjout < NBpixact_jjout; pixjjout++)
        {
            ...
            {
                uint32_t jjin = jjinarrayActive[pixjjin];
                float pha = 2.0 * dir * M_PI * (YinarrayActive[pixjjin] * YoutarrayActive[pixjjout]);
                float cospha = cosf(pha);
                float sinpha = sinf(pha);
 ...
 *
 */
errno_t fft_DFTinsertFPM(const char *pupin_name, const char *fpmz_name, double zfactor, const char *pupout_name,
                         imageID *outID)
{
    DEBUG_TRACE_FSTART();
    ...
    for (uint32_t k = 0; k < zsize; k++) // increment slice (= wavelength)
    {
        ...
        FUNC_CHECK_RETURN(fft_DFT(pupin_name, "_DFTpupmask", "_foc0", "_fpmzmask", zfactor, -1, k, &ID));
        ...
        */
        ...
        FUNC_CHECK_RETURN(fft_DFT("_foc0", "_fpmzmask", "_pupout2D", "_DFTpupmask", zfactor, 1, 0, &IDout2D));
        ...
        for (uint64_t ii = 0; ii < xysize; ii++)
        {
            data.image[IDout].array.CF[k * xysize + ii].re = data.image[IDout2D].array.CF[ii].re / (xysize);
            data.image[IDout].array.CF[k * xysize + ii].im = data.image[IDout2D].array.CF[ii].im / (xysize);
        }
...
//
//
errno_t fft_DFTinsertFPM_re(const char *pupin_name, const char *fpmz_name, double zfactor, const char *pupout_name,
                            imageID *outID)
{
    DEBUG_TRACE_FSTART();
    ...
    FUNC_CHECK_RETURN(fft_DFT(pupin_name, "_DFTpupmask", "_foc0", "_fpmzmask", zfactor, -1, 0, &ID));
    ...
    FUNC_CHECK_RETURN(fft_DFT("_foc0", "_fpmzmask", pupout_name, "_DFTpupmask", zfactor, 1, 0, &IDout));
